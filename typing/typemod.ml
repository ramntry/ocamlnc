  ...
  let rec type_struct env sstr =
    ...
    match sstr with
      [] ->
        ([], [], env)
    | sitem :: srem ->
        let (str_items, sig_items, newenv) =
          type_struct_item env sitem in
        let (str_rem, sig_rem, final_env) =
          type_struct (enrich_env anchor sig_items newenv) srem in
        (str_items @ str_rem, sig_items @ sig_rem, final_env)
  and type_struct_item env sitem =
    match sitem with
    | {pstr_desc = Pstr_eval sexpr} ->
        let expr = Typecore.type_expression env sexpr in
        ([Tstr_eval expr],
         [],
         env)
    | {pstr_desc = Pstr_value(rec_flag, sdefs)} ->
        let (defs, newenv) =
          Typecore.type_binding env rec_flag sdefs in
        let bound_idents = let_bound_idents defs in
        let make_sig_value id =
        ...
        ([Tstr_value(rec_flag, defs)],
         List.map make_sig_value bound_idents,
         newenv)
    | {pstr_desc = Pstr_primitive(name, sdesc)} ->
        let desc = Typedecl.transl_value_decl env sdesc in
        let (id, newenv) = Env.enter_value name desc env in
        ([Tstr_primitive(id, desc)],
         [Tsig_value(id, desc)],
         newenv)
    | {pstr_desc = Pstr_type sdecls; pstr_loc = loc} ->
        List.iter
          ...
          sdecls;
        let (decls, newenv) = Typedecl.transl_type_decl env sdecls in
        let items = List.map (fun (id, info) -> Tsig_type(id, info)) decls in
        ([Tstr_type decls],
         List.map (fun (id, info) -> Tsig_type(id, info)) decls,
         newenv)
    | {pstr_desc = Pstr_exception(name, sarg)} ->
        let arg = Typedecl.transl_exception env sarg in
        let (id, newenv) = Env.enter_exception name arg env in
        ([Tstr_exception(id, arg)],
         [Tsig_exception(id, arg)],
         newenv)
    | {pstr_desc = Pstr_exn_rebind(name, longid); pstr_loc = loc} ->
        let (path, arg) = Typedecl.transl_exn_rebind env loc longid in
        let (id, newenv) = Env.enter_exception name arg env in
    ...
    | {pstr_desc = Pstr_module(name, smodl); pstr_loc = loc} ->
        check "module" loc module_names name;
        let modl = type_module (anchor_module name anchor) env smodl in
        let (id, newenv) = Env.enter_module name modl.mod_type env in
        ([Tstr_module(id, modl)],
         [Tsig_module(id, modl.mod_type)],
         newenv)
    | {pstr_desc = Pstr_recmodule sbind; pstr_loc = loc} ->
        List.iter
          (fun (name, _, _) -> check "module" loc module_names name)
          sbind;
        let (decls, newenv) =
        ...
        let type_recmodule_binding (id, mty) (name, smty, smodl) =
          ...
          let coercion =
            try
              Includemod.modtypes
                     (Env.add_module id modl.mod_type newenv)
                     modl.mod_type mty
            with Includemod.Error msg ->
              raise(Error(smodl.pmod_loc, Not_included msg)) in
          let modl' =
        ...
        let bind = List.map2 type_recmodule_binding decls sbind in
        ([Tstr_recmodule bind],
         List.map (fun (id, modl) -> Tsig_module(id, modl.mod_type)) bind,
         newenv)
    | {pstr_desc = Pstr_modtype(name, smty); pstr_loc = loc} ->
        ...
        ([Tstr_modtype(id, mty)],
         ...
         newenv)
    | {pstr_desc = Pstr_open lid; pstr_loc = loc} ->
        let (path, mty) = type_module_path env loc lid in
        let sg = extract_sig_open env loc mty in
        ([], [], Env.open_signature path sg env)
    | {pstr_desc = Pstr_class cl; pstr_loc = loc} ->
         List.iter
           (fun {pci_name = name} -> check "type" loc type_names name)
           cl;
        ...
        ([Tstr_class
         ...
         List.flatten
           (List.map
              ...
              classes),
         newenv)
    | {pstr_desc = Pstr_class_type cl; pstr_loc = loc} ->
        List.iter
          (fun {pci_name = name} -> check "type" loc type_names name)
          cl;
        let (classes, newenv) = Typeclass.class_type_declarations env cl in
        ([Tstr_cltype
         ...
         List.flatten
           (List.map
              ...
              classes),
         newenv)
    | {pstr_desc = Pstr_include smodl; pstr_loc = loc} ->
        ...
        List.iter
          (check_sig_item type_names module_names modtype_names loc) sg;
        let newenv = Env.add_signature sg env in
        ([Tstr_include (modl, bound_value_identifiers sg)],
         sg,
         newenv)
  in type_struct env sstr
***)
