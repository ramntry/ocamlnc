...
let ctx_matcher p =
  ...
  | Tpat_construct (_, _, cstr,omegas) ->
      ...
      | Tpat_construct (_, _, cstr',args) when cstr.cstr_tag=cstr'.cstr_tag ->
          p,args @ rem
...
let rec explode_or_pat arg patl mk_action rem vars aliases = function
  | {pat_desc = Tpat_or (p1,p2,_)} ->
      explode_or_pat
        ...
        vars aliases p1
  | {pat_desc = Tpat_alias (p,TPat_alias (id, _))} ->
...
| Tpat_construct (_, _, _, _) -> group_constructor
...
pat_desc = Tpat_or (a1, a2, None)}::
    ...
    | Tpat_construct (_, _, cstr1, [arg]) when cstr.cstr_tag = cstr1.cstr_tag ->
        arg::rem
    | Tpat_any -> omega::rem
    | _ -> raise NoMatch in
    matcher_rec
